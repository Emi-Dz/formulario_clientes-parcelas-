...
export const generateExcel = (saleData: SaleData): void => {
    ...
    const main_data: (string | number | Date | XLSX.CellObject)[][] = [
        ...
        ['Idiomas', Object.entries(saleData.languages).filter(([,v]) => v).map(([k]) => k.toUpperCase()).join(', ')],
        [], // Spacer
        ...
        ['Precio Total', { v: `R$ ${saleData.totalProductPrice}`, t: 's', s: { alignment: { horizontal: 'left' } } }],
        ['Anticipo', { v: `R$ ${saleData.downPayment}`, t: 's', s: { alignment: { horizontal: 'left' } } }],
        ['Cuotas', `${saleData.installments} de R$ ${saleData.installmentPrice.toFixed(2)}`],
    ...
    // Main data merges and key styling
    for (let R = 0; R < main_data.length; ++R) {
        ...
        if (row.length > 1) {
             ...
             if (ws[cell_ref] && ws[cell_ref].t) {
                const currentStyle = ws[cell_ref].s || {};
                const currentFont = (currentStyle as any).font || {};
                 ws[cell_ref].s = { ...currentStyle, font: { ...currentFont, bold: true } };
             }
        }
    }
