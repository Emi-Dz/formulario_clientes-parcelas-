...
export const DataEntryForm: React.FC<DataEntryFormProps> = ({ initialData, onSubmit, onCancel, isLoading, isEditMode }) => {
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 p-4 md:p-6 rounded-xl shadow-lg space-y-6">
            ...
            <div className="pt-4 flex justify-end space-x-4">
                ...
                <button
                    type="submit"
                    disabled={isLoading}
                    className="flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:bg-slate-400 disabled:cursor-not-allowed"
                >
                    {isLoading ? (
                        <>
                        ...
                        isEditMode ? t('saveChanges') : t('submitAndGenerate')
                    )}
                </button>
            </div>
        </form>
